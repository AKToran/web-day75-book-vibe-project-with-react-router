...
const BookDetails = () => {
  ...
  const { bookName, author, image, tags, rating, category, yearOfPublishing, publisher, totalPages, review } = book;
  return (
    <div className='flex flex-col md:flex-row my-12'>
      ...
      <div className='p-8 md:w-1/2 space-y-6'>
        ...
        <hr />
        <p>Review: {review}</p>
        <h4><span className='font-bold'>Tag:</span> {tags.map(((t, i)=> <span key={i} className='me-2'>{t}</span>))}</h4>
        <hr />
        <table>
          <tbody>
            <tr>
        ...
        <button className='btn btn-accent btn-outline me-4 mb-0'>Read Now</button>
